...
from obspy.io.dirtree.sds import SDS_FMTSTR, read_from_SDS
    ...
    def test_read_from_SDS(self):
        ...
        for year, doy in ((2015, 123), (2015, 1), (2012, 1)):
            ...
            with TemporarySDSDirectory(year=year, doy=doy) as temp_sds:
                ...
                st = read_from_SDS(temp_sds.tempdir, seed_id, t-20, t+20)
                self.assertEqual(len(st), 1)
                self.assertEqual(st[0].stats.starttime, t-20)
                self.assertEqual(st[0].stats.endtime, t+20)
                self.assertEqual(len(st[0]), 5)
                # test that disabling cleanup option works
                st = read_from_SDS(temp_sds.tempdir, seed_id, t-200, t+200,
                                   cleanup_merge=False)
                ...
                st = read_from_SDS(temp_sds.tempdir, seed_id, t-80, t-30)
                self.assertEqual(len(st), 1)
                # test reading data from a single day file
                # (data is in the dayfile of the previous day)
                st = read_from_SDS(temp_sds.tempdir, seed_id, t+20, t+40)
                self.assertEqual(len(st), 1)
